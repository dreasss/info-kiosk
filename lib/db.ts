    ...
    try {
      ...
      request.onerror = (event) => {
        ...
        if (errorMessage.includes('quota') || errorMessage.includes('storage')) {
          ...
          console.error("Запрос к базе данных был прерван - возможно конфликт с другими запросами")
        }

        safeReject(new Error(`Не удалось открыть базу данных: ${errorMessage}`))
      }
      ...
      request.onupgradeneeded = (event) => {
        try {
          ...
          console.log(`Обновление базы данных с версии ${event.oldVersion} до ${event.newVersion}`)
          ...
          if (!db.objectStoreNames.contains(STORES.ICONS)) {
            const iconsStore = db.createObjectStore(STORES.ICONS, { keyPath: "id" })
            iconsStore.createIndex("category", "category", { unique: false })
          }

          if (!db.objectStoreNames.contains(STORES.RSS_FEEDS)) {
            const rssStore = db.createObjectStore(STORES.RSS_FEEDS, { keyPath: "id" })
          ...
          transaction.onabort = (txAbort) => {
            console.error("Транзакция обновления схемы была прервана:", txAbort)
      ...
      request.onblocked = (event) => {
        console.warn("База данных заблокирована другой вкладкой. Попробуйте закрыть другие вкладки приложения.")
        safeReject(new Error("База данных заблокирована другой вкладкой"))
      }

    } catch (error) {
      console.error("Ошибка при создании запроса к IndexedDB:", error)
      safeReject(new Error("Не удалось создать запрос к IndexedDB"))
    }
  }
}
    ...
    return new Promise((resolve, reject) => {
      poisRequest.onsuccess = () => {
        ...
        if (count === 0) {
          ...
          console.log(`База данных содержит ${count} POI, демо-данные не нужны`)
          resolve()
        }
      }
...
async function initializeDemoDataSeparately(db: IDBDatabase): Promise<void> {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([STORES.POIS, STORES.NEWS, STORES.RSS_FEEDS, STORES.SETTINGS], "readwrite")
  ...
  try {
    ...
    try {
      ...
      demoPOIs.forEach((poi) => {
        ...
        request.onerror = (e) => console.warn("Ошибка добавления POI:", poi.id, e)
      })
    } catch (error) {
    ...
    try {
      ...
      demoNews.forEach((news) => {
        ...
        request.onerror = (e) => console.warn("Ошибка добавления новости:", news.id, e)
      })
    } catch (error) {
    ...
    try {
      ...
      demoRssFeeds.forEach((feed) => {
        ...
        request.onerror = (e) => console.warn("Ошибка добавления RSS ленты:", feed.id, e)
      })
    } catch (error) {
...
export async function getIconsByCategory(category: string): Promise<MarkerIcon[]> {
  const db = await initDB()
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(STORES.ICONS, "readonly")
    const store = transaction.objectStore(STORES.ICONS)
...
export async function saveIcon(icon: MarkerIcon, iconBlob?: Blob): Promise<MarkerIcon> {
  const db = await initDB()
  return new Promise((resolve, reject) => {
