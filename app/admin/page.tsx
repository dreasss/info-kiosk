...
import {
  Home, Save, Trash, Plus, ImageIcon, Settings, MapPin, Newspaper,
  Building, Info, Rss, LogOut, Upload, Edit, X, Check
} from "lucide-react"
...
import {
  fetchPOIs, createPOI, updatePOI, removePOI,
  fetchNews, createNews, updateNews, removeNews,
  fetchMedia, removeMediaItem, fetchAlbums, createAlbum, updateAlbum, removeAlbum,
  fetchSettings, updateSettings,
  fetchRssFeeds, createRssFeed, updateRssFeed, removeRssFeed,
  fetchIcons, createIcon, updateIcon, removeIcon,
  uploadMediaFile
} from "@/lib/api"
...
export default function AdminPage() {
  ...
  const loadData = async () => {
    setIsLoading(true)
    try {
      const [poisData, newsData, settingsData, mediaData, albumsData, rssData, iconsData] =
        await Promise.all([
          fetchPOIs(),
          fetchNews(),
          fetchSettings(),
  ...
  const handleSavePoi = async () => {
    try {
      ...
      setPoiForm({ name: "", shortDescription: "", fullDescription: "", address: "", category: "building", coordinates: [56.7417, 37.189] })
      setEditingPoi(null)
      loadData()
    } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600">Загрузка панели администратора...</p>
        </div>
      </div>
    )
  }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="pois" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>{editingPoi ? "Редактировать объект" : "Добавить новый объект"}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setPoiForm({ ...poiForm, shortDescription: e.target.value })}
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setPoiForm({ ...poiForm, fullDescription: e.target.value })}
                  ...
                  <div>
                    ...
                    <Select value={poiForm.category} onValueChange={(value) => setPoiForm({ ...poiForm, category: value })}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setPoiForm({
                          ...poiForm,
                          coordinates: [parseFloat(e.target.value), poiForm.coordinates[1]]
                        })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setPoiForm({
                          ...poiForm,
                          coordinates: [poiForm.coordinates[0], parseFloat(e.target.value)]
                        })}
                      />
                    </div>
                  </div>
                    ...
                    {pois.map((poi) => (
                      <div key={poi.id} className="flex items-center justify-between p-3 border rounded-lg">
                        ...
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" onClick={() => handleEditPoi(poi)}>
                            <Edit className="h-3 w-3" />
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => handleDeletePoi(poi.id)}>
          ...
          <TabsContent value="news" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>{editingNews ? "Редактировать новость" : "Добавить новость"}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    ...
                    {news.map((newsItem) => (
                      <div key={newsItem.id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600 line-clamp-2">{newsItem.content}</p>
            ...
            <MediaManager onDataChange={loadData} />
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) => setRssForm({ ...rssForm, active: checked })}
                    />
                    <Label>Активна</Label>
                  </div>
                </CardContent>
                    ...
                    {rssFeeds.map((feed) => (
                      <div key={feed.id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex-1">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${feed.active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                            {feed.active ? 'Активна' : 'Неактивна'}
                          </span>
                        </div>
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" onClick={() => handleEditRss(feed)}>
                            <Edit className="h-3 w-3" />
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => handleDeleteRss(feed.id)}>
                            <Trash className="h-3 w-3" />
                          </Button>
                        </div>
                      </div>
          ...
          <TabsContent value="icons" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <Label>Категория</Label>
                  <Select value={iconForm.category} onValueChange={(value) => setIconForm({ ...iconForm, category: value })}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {icons.map((icon) => (
                    <div key={icon.id} className="border rounded-lg p-3 text-center">
                      ...
                      <p className="text-xs text-gray-500">{CATEGORIES[icon.category]?.name}</p>
                      <Button
                        size="sm"
                        variant="outline"
            ...
            {settings && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => handleOrganizationChange('fullName', e.target.value)}
                      />
                    </div>
                    <div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      <Label htmlFor="idle-timeout">Время бездействия (минуты)</Label>
                      <Input
                        id="idle-timeout"
                        type="number"
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        Время бездействия после которого система вернется на главный экран
                      </p>
                    </div>
