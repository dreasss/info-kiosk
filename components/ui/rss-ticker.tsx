...
export function RssTicker({ className }: RssTickerProps) {
  ...
  const getDemoNews = (): RssItem[] => [
    {
      title: "ОИЯИ: Открытие нового сверхтяжелого элемента в лаборатории ядерных реакций",
      link: "#",
    ...
    {
      title: "Прорыв в области физики элементарных частиц: обнаружена новая частица",
      link: "#",
      pubDate: new Date(Date.now() - 7200000).toISOString(),
      source: "Physics Today",
    },
    {
      title: "Дубна станет центром международных исследований в области ядерной физики",
      link: "#",
      pubDate: new Date(Date.now() - 10800000).toISOString(),
      source: "JINR News",
    },
    {
      title: "Новый ускоритель частиц NICA готовится к запуску первых экспериментов",
      link: "#",
  ...
  useEffect(() => {
    ...
    const fetchRssNews = async () => {
        ...
        console.log("RSS Ticker: Found active feeds:", activeFeeds.length, activeFeeds)
          ...
          try {
            ...
            if (data.contents) {
              ...
              items.forEach((item, index) => {
                if (index < 5) { // Берем только первые 5 новостей с каждой ленты
        ...
        if (allNewsItems.length > 0) {
          ...
          allNewsItems.sort((a, b) => new Date(b.pubDate || 0).getTime() - new Date(a.pubDate || 0).getTime())
          console.log("RSS Ticker: Successfully loaded", allNewsItems.length, "news items")
          setNews(allNewsItems.slice(0, 10))
        } else {
          // Fallback на демо-данные
          console.log("RSS Ticker: No RSS items loaded, showing fallback message")
          const mockNews: RssItem[] = [
            {
              title: "Настройте RSS ленты в панели администратора для отображения актуальных новостей",
              link: "/admin",
              pubDate: new Date().toISOString(),
              source: "Система",
            },
        ...
        if (demoData.length > 0) {
          ...
          setNews([
            {
              title: "Ошибка загрузки RSS лент. Проверьте настройки в панели администратора.",
              link: "/admin",
              pubDate: new Date().toISOString(),
              source: "Система",
            }
      ...
      <div className="bg-gradient-to-r from-blue-500/90 via-blue-600/95 to-sky-500/90 backdrop-blur-sm text-white py-4 overflow-hidden border-y border-white/10">
        <div className="flex items-center h-8">
          <div className="font-bold mr-8 px-6 py-2 whitespace-nowrap bg-white/15 backdrop-blur-md rounded-r-2xl flex items-center shadow-lg border border-white/20">
            ...
            <span className="font-semibold tracking-wide">ЗАГРУЗКА НОВОСТЕЙ...</span>
          </div>
        </div>
      </div>
    )
          ...
          <div className="ticker-container overflow-hidden relative w-full">
            <div className="ticker-content whitespace-nowrap animate-scroll-slow">
              {demoNews.concat(demoNews).map((item, index) => (
                <span key={index} className="mx-12 font-medium hover:text-blue-200 cursor-pointer transition-all duration-300 hover:scale-105 inline-block">
                  <span className="text-yellow-300 mr-3 text-lg">🔬</span>
                  <span className="font-semibold text-blue-100">{item.source}</span>
  ...
  return (
    <div className={cn("bg-gradient-to-r from-blue-500/90 via-blue-600/95 to-sky-500/90 backdrop-blur-sm text-white py-4 overflow-hidden border-y border-white/10", className)}>
      <div className="flex items-center h-8">
        <div className="font-bold mr-8 px-6 py-2 whitespace-nowrap bg-white/15 backdrop-blur-md rounded-r-2xl flex items-center shadow-lg border border-white/20"
          style={{
